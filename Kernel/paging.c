...
void init_paging(void) {
    ...
    pdpe_table1[0]  = create_pdpe(0x00, PAGE_US | PAGE_RW | PAGE_PRESENT );
    pdpe_table1[1]  = create_pdpe(0x01, PAGE_US | PAGE_RW | PAGE_PRESENT );
    pdpe_table1[2]  = create_pdpe(0x02, PAGE_US | PAGE_RW | PAGE_PRESENT );
    pdpe_table1[3]  = create_pdpe(0x03, PAGE_US | PAGE_RW | PAGE_PRESENT );

    _asm_set_cr3(cr3);
}

PML4E get_pml4e(int pml4e_index){
    PML4E * table = get_pml4_table();
    return table[pml4e_index];
}

void set_pml4e(PML4E entry, int pml4_index) {
    PML4E * table = get_pml4_table();
